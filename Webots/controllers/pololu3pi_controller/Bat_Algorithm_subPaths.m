%clear
%clc
function [t, trajectoryBats, obstacles, pathCost] = Bat_Algorithm_subPaths(PopulationSize, NumMovements, Dimension, Gamma, Alpha, graphs, start, goal, map, numSubPaths, cost)
    %% Bat Algorithm
    ...
    [t1, ~, ~, ~, firstSubPath, ~] = Bat_Algorithm_Path(PopulationSize, NumMovements, Dimension, Gamma, Alpha, x_rangeFirst, y_rangeFirst, start, [realFirstGoalx, realFirstGoaly], map, cost, obstacles);
    t = t + t1;
    ...
    for i = 1:(numSubPaths-1)
        ...
        [tn, ~, ~, ~, nSubPath, ~] = Bat_Algorithm_Path(PopulationSize, NumMovements, Dimension, Gamma, Alpha, nXrange, nYrange, nStart, [nGoalx, nGoaly], map, cost, obstacles);
        t = t + tn;
        subPaths(1+i,:) = nSubPath;
    end
    
    trajectoryBats = subPaths;
    pathCost = getDistancePath(trajectoryBats);
    figure(4); clf;
    hold on;
    axis equal; grid on;
    xlabel('X'); ylabel('Y');
