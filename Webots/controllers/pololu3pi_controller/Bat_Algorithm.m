%clear
%clc
function [t, movements, Best, AverageSolution, LastValueOf_N, trajectoryBats] = Bat_Algorithm(PopulationSize, NumMovements, Dimension, Gamma, Alpha, optimiser, graphs, checkRadius)
    %% Bat Algorithm
    ...
    for i = 1:PopulationSize
       ...
       Fitness(i,1) = optimisation_functions(Population(i,1), Population(i,2), optimiser, "Optimiser");
    end
    ...
    if graphs == 2
        gif_filename = 'bat_algorithm.gif';  % name for the GIf
    ...
    for movements = 1:NumMovements
        ...
        for bat = 1:PopulationSize
            ...
            %Update the fitness
            [FitnessValue, ~] = optimisation_functions(Position(1), Position(2), optimiser, "Optimiser");
            ...
            checkError = breakCheck(Population(bat, 1), Population(bat, 2), optimiser, checkRadius);
            N = N + checkError;
        end
        ...
        for order = 1:PopulationSize
            originalBatIndex = Index(order);  % índice original del bat
    ...
    if graphs == 1
        ...
        fprintf("Best Solution is: x = %.5f, y = %.5f with a fitness of %.7f \n%", Population(ind, 1), Population(ind, 2), Best);
